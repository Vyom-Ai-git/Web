...
const AgenticCover: React.FC = () => (
  <svg viewBox="0 0 400 200" className="w-full h-full stroke-primary/30 fill-none opacity-85" xmlns="http://www.w3.org/2000/svg">
    ...
    <circle cx="200" cy="20" r="4.5" className="fill-[#9fff00] stroke-[#1a1a1a]/40" strokeWidth="0.5" />
...
const AuditCover: React.FC = () => (
  <svg viewBox="0 0 200 120" className="w-full h-full stroke-primary/30 fill-none opacity-80" xmlns="http://www.w3.org/2000/svg">
    ...
    <circle cx="100" cy="60" r="30" strokeWidth="0.75" className="stroke-[#9fff00]" />
...
const GrowthCover: React.FC = () => (
  <svg viewBox="0 0 200 120" className="w-full h-full stroke-primary/30 fill-none opacity-80" xmlns="http://www.w3.org/2000/svg">
    ...
    <circle cx="150" cy="60" r="4" className="fill-[#9fff00] stroke-[#1a1a1a]/40" strokeWidth="0.5" />
...
const RagCover: React.FC = () => (
  <svg viewBox="0 0 200 120" className="w-full h-full stroke-primary/30 fill-none opacity-80" xmlns="http://www.w3.org/2000/svg">
    ...
    <circle cx="175" cy="60" r="4" className="fill-[#9fff00] stroke-[#1a1a1a]/40" strokeWidth="0.5" />
...
export const Insights: React.FC = () => {
  ...
  const filteredArticles = activeCategory === 'All'
    ? articles.slice(1) // exclude featured article from lower grid in All view
    : articles.filter(art => art.category === activeCategory)

  // Featured article matches the first article
        ...
        <div className="grid grid-cols-12 gap-x-4 md:gap-x-8">
          <div className="col-span-12 md:col-span-10 md:col-start-2 text-left flex flex-col gap-4">
            ...
            <p className="font-sans text-[15px] md:text-[16.5px] leading-relaxed text-[#8e8e8e] max-w-[680px]">
              Technology evolves quickly, but enduring advantage comes from understanding how systems, people, and strategy evolve together. Explore research, implementation guides, and perspectives from the Vyoma team.
            </p>
          </div>
        </div>

        {/* ─── FEATURED INSIGHT (Cover story format) ───────────────────────────── */}
        {activeCategory === 'All' && (
          <motion.div
            ...
            className="grid grid-cols-12 gap-y-8 lg:gap-x-12 items-center border-t border-black/[0.05] pt-10"
          >
            {/* Left abstract banner cover */}
            <div className="col-span-12 lg:col-span-6 py-10 bg-white/20 border border-black/[0.03] rounded-[8px] flex items-center justify-center shadow-soft">
              {renderCover(featuredArticle.coverType)}
            </div>

            {/* Right text layout metadata */}
            <div className="col-span-12 lg:col-span-6 flex flex-col gap-5 text-left">
              <div className="flex items-center gap-4 text-[11px] font-sans font-bold text-[#8e8e8e] uppercase tracking-wider">
                <span className="text-[#1a1a1a]">{featuredArticle.category}</span>
                <span>•</span>
              ...
              <h3 className="font-display font-medium text-[28px] md:text-[36px] text-[#1a1a1a] tracking-tight leading-snug">
                {featuredArticle.title}
              </h3>
              
              <p className="font-sans text-[14.5px] md:text-[15.5px] leading-relaxed text-[#8e8e8e] max-w-[500px]">
                {featuredArticle.summary}
              </p>

              <div className="flex items-center gap-3.5 mt-2">
                <span className="text-[12px] font-sans font-bold text-[#1a1a1a]">
                  BY {featuredArticle.author.toUpperCase()}
                </span>
                <a 
                  href={`#insight-${featuredArticle.slug}`}
                  className="font-sans text-[13.5px] text-[#8e8e8e] hover:text-[#1a1a1a] hover-link-underline lowercase transition-colors inline-block"
                >
                  read insight →
                </a>
              </div>
            ...
            {categories.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`px-4 py-2 rounded-pill font-sans text-[13px] transition-all border-none outline-none shrink-0 cursor-pointer ${
                    isActive 
            ...
            <AnimatePresence mode="wait">
              ...
              >
                {filteredArticles.map((art) => (
                  <article 
                    key={art.id}
                    className="flex flex-col gap-4 p-6 rounded-[6px] border border-black/[0.03] bg-white/25 hover:border-black/10 hover:shadow-soft transition-all duration-300 group text-left relative"
                  >
                    {/* Small cover display */}
                    <div className="w-full flex items-center justify-center py-6 bg-white/20 border border-black/[0.02] rounded-[4px] mb-2 pointer-events-none group-hover:opacity-100 transition-opacity">
                      {renderCover(art.coverType)}
                    </div>

                    <div className="flex items-center gap-3 text-[10.5px] font-sans font-bold text-[#8e8e8e] uppercase tracking-wider">
                      <span className="text-[#1a1a1a]">{art.category}</span>
                      <span>•</span>
                      <span>{art.readingTime}</span>
                    </div>

                    <h4 className="font-display font-medium text-[19px] md:text-[21px] text-[#1a1a1a] leading-snug tracking-tight group-hover:underline">
                      <a href={`#insight-${art.slug}`} className="outline-none">
                        {art.title}
                      </a>
                    </h4>
                    ...
                    <div className="flex items-center justify-between text-[11px] font-sans text-[#8e8e8e] mt-4 border-t border-black/[0.03] pt-3.5">
            ...
            {resources.map((res) => (
              ...
              >
                ...
                <div className="flex items-center justify-between border-t border-black/[0.03] pt-3">
                  ...
                  <a
                    href="#download"
                    className="text-[12px] font-sans font-bold text-[#1a1a1a] hover:text-[#9fff00] group-hover:underline lowercase transition-colors"
                  >
                    {res.cta.toLowerCase()} →
                  </a>
                </div>
            ...
            <AnimatePresence mode="wait">
              {!subscribed ? (
                ...
                <motion.div 
                  ...
                  className="text-[13.5px] font-sans font-bold text-[#1a1a1a] bg-[#9fff00]/15 border border-[#9fff00]/30 py-3.5 px-6 rounded-pill text-center shadow-sm"
                  initial={{ opacity: 0, scale: 0.95 }}
                  animate={{ opacity: 1, scale: 1 }}
                  transition={{ duration: 0.3 }}
                >
                  Subscription verified. Welcome to Vyoma Intelligence.
                </motion.div>
              )}
            </AnimatePresence>
          </div>
