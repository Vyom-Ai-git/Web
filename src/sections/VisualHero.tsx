  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none z-0">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[700px] bg-[#ef233c]/[0.07] rounded-full blur-[140px]" />
        <div className="absolute inset-0 technical-grid opacity-25 [mask-image:radial-gradient(circle_at_center,black_40%,transparent_80%)]" />
        
        {/* Giant subtle emblem watermark without text in the background */}
        <div className="absolute right-4 md:right-16 top-1/2 -translate-y-1/2 opacity-[0.14] pointer-events-none hidden lg:block">
          <OnnrevSymbolMark size={520} />
        </div>
      </div>
      ...
      <div className="max-w-6xl w-full mx-auto px-6 md:px-12 relative z-10">
        ...
        >
          ...
          <span className="text-xs font-mono font-medium text-red-100/90 tracking-wider uppercase">
            ONNREV 2026 // PRODUCTION SYSTEMS ARCHITECTURE
          </span>
        </motion.div>
      </div>
      ...
      <div className="max-w-6xl w-full mx-auto px-6 md:px-12 relative z-10 my-auto py-12 text-left">
        <div className="max-w-[1050px] flex flex-col gap-6 md:gap-8">
          ...
          {/* 3 to 8 words headline with Red Accent Underline */}
          <motion.h1
            initial={reducedMotion ? { opacity: 1 } : { opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ ...transition, delay: 0.2 }}
            className="font-display font-semibold text-[52px] sm:text-[76px] md:text-[96px] lg:text-[116px] leading-[0.95] tracking-tight text-white uppercase text-balance"
          >
            <span className="block text-transparent bg-clip-text bg-gradient-to-b from-white via-white to-white/50">
              Engineering
            </span>
            <span className="block text-transparent bg-clip-text bg-gradient-to-b from-white via-white to-white/50">
              What <span className="text-[#ef233c] inline-block relative">
                Matters.
          ...
          {/* 1 short supporting sentence (under 15 words) */}
          <motion.p
            initial={reducedMotion ? { opacity: 1 } : { opacity: 0, y: 15 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ ...transition, delay: 0.3 }}
            className="font-sans text-[18px] md:text-[22px] text-zinc-400 font-light max-w-[620px] leading-relaxed"
          >
            Resilient intelligent infrastructure and closed-loop autonomous workflows for modern enterprises.
          </motion.p>

          {/* Primary Action Button (Shiny CTA Red) */}
          <motion.div
            initial={reducedMotion ? { opacity: 1 } : { opacity: 0, y: 15 }}
            animate={{ opacity: 1, y: 0 }}
